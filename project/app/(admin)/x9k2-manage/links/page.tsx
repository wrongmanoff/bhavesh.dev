    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/x9k2-manage/links" className="font-mono text-xs text-[#6b6b6b] hover:text-[#00ff88]">← back</Link>
        </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/x9k2-manage/links?new=1" className="font-mono text-xs px-3 py-1.5 rounded border border-[#00ff88]/40 text-[#00ff88]">+ new link</Link>
      </div>
      <div className="space-y-2">
        {(links as SiteLink[] | null)?.map((l) => (
          <Link key={l.id} href={`/x9k2-manage/links?edit=${l.id}`} className="block p-4 rounded-lg border border-[#1e1e1e] bg-[#111111] hover:border-[#2e2e2e]">
