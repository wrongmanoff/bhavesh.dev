    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/admin/vault" className="font-mono text-xs text-[#6b6b6b] hover:text-[#00ff88]">← back</Link>
        </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/vault?new=1" className="font-mono text-xs px-3 py-1.5 rounded border border-[#00ff88]/40 text-[#00ff88]">+ new</Link>
      </div>
      <div className="space-y-2">
        {(items as Achievement[] | null)?.map((a) => {
          const cfg = ACHIEVEMENT_TYPE_CONFIG[a.type];
          return (
            <Link key={a.id} href={`/admin/vault?edit=${a.id}`} className="block p-4 rounded-lg border border-[#1e1e1e] bg-[#111111] hover:border-[#2e2e2e]">
